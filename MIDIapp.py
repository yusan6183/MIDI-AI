...
使われたコード進行はすべて記述して下さい。

あなたは音楽理論の教師です。
上記の結果を踏まえて学習者向けに各項目をーーーーーーーーーーーーーーーーーーーーー線で区切って、ユーザーの意図とキーに使われた言語に合わせて優しくフィードバックしてください。
フィードバックでコードを提示する際、Cm[C、E-、G]のように構成音も必ず記述してください。

以下の順でフィードバックを記述してください
・まとめ（改善点と改善案の要点をまとめてください）

以下の項目で採点をミスがないように行ってください
・キーとの適合性、ダイアトニック内のコード数/使われた全コード数*20 最大20点　適合していないコードの解説も含めてください
・バリエーション、使われたコードの種類/使われた全コード数*20　最大20点
・終止の美しさ、コード進行の最初がトニック（Iかviｍ）かサブドミナント（IVかiiｍ）で始まると＋5点、最後の2コードがドミナント（iiiｍかV）→トニック（Iかviｍ）の場合は+5点　最大10点
・滑らかさ、コードの間の増減が５半音以内であればそれぞれ5点、全部が５半音以内ならさらに5点　最大20点
・構成のまとまり、同じコード進行が繰り返されていれば+10点　
・方向性の一致、ユーザーの意図とキーに合致しているか　キーは+5点　意図は+15点　最大20点
・総合スコア　スコア/100点での表示
計算結果が正しいかの見直しを行ってください。計算が間違っていたら訂正を行ってください。
採点項目で満点でない部分は、その項目に関連する音楽理論を教えてください。また、採点項目ごとに満点をこえないようにしてください。

以下はまとめとは別に詳細に記述してください
